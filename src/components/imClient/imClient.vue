      ...
      <div class="imClient-inner">
        <header class="imClient-header">
          <div class="name-wrapper position-v-mid">
            <span v-if="chatInfoEn.chatState == 'robot'">红星培优在线客服-客户端</span>
        ...
        <main class="imClient-main">
          ...
          <div class="item imClientChat-wrapper">
            ...
            <common-chat ref="common_chat" :chatInfoEn="chatInfoEn" :oprRoleName=" 'client'" @sendMsg="sendMsg"
                         @chatCallback="chatCallback"></common-chat>
          </div>
          <!-- 信息区域 -->
...
<script>
  ...
  import {getUsuallyQuestionList} from '@/api/user.js';
  export default {
    components: {
    ...
    mounted () {
      getUsuallyQuestionList().then((response)=>{
...
.bbb{
  background: url("../../assets/wechatbackground.jpg");
